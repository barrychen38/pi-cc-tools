...
- Collapsed successful results now show the first 3 width-bounded output lines in muted gray with a dim `… +N lines (ctrl+o to expand)` hint instead of a bare line count; applies to built-in and MCP/custom tools alike.
- Running `bash` tools now show a live 3-line tail preview with elapsed time, while completed rows retain a dim duration.
